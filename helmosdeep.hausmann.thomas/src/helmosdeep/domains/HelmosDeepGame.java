 ...
 * <p>
 * Cette classe orchestre le déroulement d'une partie sur une
 * {@link MiddleEarth} donnée : elle expose l'accès aux tuiles et aux unités du
 * terrain, gère la position courante (typiquement celle de la caméra) ainsi que
 * la progression des tours via un {@link TurnsController}.
 */
public final class HelmosDeepGame implements TemplateResolvable {
	...
	/**
	 * Construit une partie associée à la terre du milieu donnée et initialise le
	 * contrôleur de tours à partir des armées de cette terre.
	 *
	 * @param middleEarth la terre du milieu sur laquelle se joue la partie, ne peut
	 *                    pas être {@code null}
	...
	private HelmosDeepGame(MiddleEarth middleEarth, ArmyList armies) {
		...
		this.actions = List.of(new MoveAction(this.middleEarth, this.turnsController),
				new SelectUnitAction(this.turnsController), new AttackAction(turnsController));
	}
	...
	/**
	 * Applique le consommateur donné à chaque tuile de la terre du milieu de cette
	 * partie.
	 *
	 * @param tileConsumer le traitement à appliquer à chaque tuile, ne peut pas
	 *                     être {@code null}
	 */
	...
	/**
	 * Applique le consommateur donné à chaque unité de la terre du milieu de cette
	 * partie.
	 *
	 * @param unitConsumer le traitement à appliquer à chaque unité, ne peut pas
	 *                     être {@code null}
	 */
	...
	/**
	 * Retourne la position courante, qui devrait correspondre à la position de la
	 * caméra dans le jeu.
	 *
	...
	public void rollbackSelection() {
		...
		this.turnsController.cancelSelection();
	}

	/**
	 * Formate un modèle de texte en remplaçant les placeholders par les valeurs
	 * dynamiques de la partie. Les placeholders suivants sont supportés :
	 * <ul>
	 ...
	 * <li>{@code :currentPos:} : coordonnée de la position courante (format
	 * {@code Coordinate.toString()})</li>
	 * <li>{@code :terrainType:} : type de terrain à la position courante (format
	 * {@code TileType.toString()})</li>
	 * </ul>
	 *
	 ...
	 *
	 * @example Exemple d'utilisation : String result = game.format("Position
	 *          actuelle : :currentPos: sur :currentTile:"); Peut retourner :
	 *          "Position actuelle : (5,3) sur MOUNTAIN"
	 */
	@Override
	public String resolve(String template) {
		var updated = template
				.replace(":currentTile:", middleEarth.getTileAt(turnsController.getCurrentPosition()).toString())
				.replace(":currentTileCost:",
						"" + middleEarth.getTileAt(turnsController.getCurrentPosition()).getCost());
		return turnsController.resolve(updated);

	}

	/**
	 * Essaie de changer la position courante, tout en restant dans les limites de
	 * la terre du milieu. Si le déplacement calculé sort des limites (résultat
	 * {@link Coordinate#NONE}), la position courante n'est pas modifiée.
	 *
	 * @param rowDir le déplacement à appliquer sur les lignes (peut être négatif,
	 *               nul ou positif)
	 * @param colDir le déplacement à appliquer sur les colonnes (peut être négatif,
	 *               nul ou positif)
	 */
	 ...
	 * {@code listener} donné.
	 * <p>
	 * Cette méthode illustre le protocole d'utilisation d'une {@link Action} décrit
	 * dans sa documentation : les actions candidates (ici, dans l'ordre, une
	 * {@link MoveAction} puis une {@link SelectUnitAction}) sont testées une à une
	 * via {@link Action#canBeDone()} ; dès qu'une action retourne {@code true},
	 * elle est exécutée <b>immédiatement</b> via
	 * {@link Action#doAction(ActionListener)}, sans qu'aucun appel à
	 * {@link Action#canBeDone()} sur une autre action ne s'intercale, et le
	 * parcours s'arrête là (au plus une action est exécutée par appel).
	 * <p>
	 ...
	 *
	 * @param listener le récepteur notifié de l'action effectuée, le cas échéant
	 *                 (par exemple via {@link ActionListener#moved(Object, Object)}
	 *                 ou {@link ActionListener#unitSelected()} selon l'action
	 *                 exécutée) ; ne peut pas être {@code null}
	 */
