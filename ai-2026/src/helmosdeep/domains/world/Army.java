...
/**
 * Représente une armée d'un {@link Belligerent}, composée d'unités positionnées
 * sur des {@link Coordinate}, d'un pot de mouvement ({@link MvtPool}) partagée
 * par les unités, et éventuellement d'un général.
 * <p>
 * Une armée ne peut posséder qu'un seul général au maximum. Elle ne peut plus
 * combattre si son général est mort ou s'il ne lui reste plus qu'une seule
 * unité.
 */
public final class Army implements TemplateResolvable {
	...
	/**
	 * Crée une nouvelle armée vide, sans unité ni général, pour le
	 * {@link Belligerent} donné.
	 *
	...
	/**
	 * Retourne l'unité de cette armée présente à la position donnée, si elle
	 * existe.
	 *
	 * @param pos la position à consulter
	 * @return l'unité présente à {@code pos}, ou {@link Optional#empty()} si aucune
	 *         unité de cette armée ne s'y trouve
	 */
	 ...
	 * @return le mouvement disponible pour cette unité, ou celui de
	 *         {@link Unit#UNKNOWN} si aucune unité ne se trouve à {@code unitPos}
	 */
	 ...
	 * @return {@code true} si {@code b} est le camp de cette armée, {@code false}
	 *         sinon
	 */
	...
	/**
	 * Récupère l'unité général de l'armée en question.
	 * 
	 * @return l'unité général, sinon, renvoie Unit.UNKNOWN si l'unité général n'a pas été
	 *         trouvée
	 */
	public Unit getGeneral() {

		return this.general.orElse(Unit.UNKNOWN);
	}

	/**
	 * Localise la position du général de cette armée.
	 *
	 * @return la position du général, ou {@link Coordinate#NONE} si cette armée n'a
	 *         pas (ou plus) de général
	 */
	...
	/**
	 * Localise la position d'une unité dans cette armée.
	 * 
	 * @param unit l'unité dont on cherche la position dans l'armée
	 * @return la coordonnée de l'unité de l'armée, renvoie Coordinate.NONE si
	 *         aucune unité n'a été trouvée.
	 */
	public Coordinate locateUnit(Unit unit) {

		Objects.requireNonNull(unit, "Arg. unit != null attendu");

		return units.getFirstOrDefault(unit, Coordinate.NONE);
	}

	/**
	 * Enrôle une nouvelle unité dans cette armée à la position donnée.
	 * <p>
	 * Si l'unité enrôlée est de type {@link UnitType#GENERAL}, elle devient le
	 * général de l'armée. La capacité de mouvement de l'armée est augmentée de la
	 * moitié du mouvement de l'unité enrôlée.
	 *
	 * @param pos  la position à laquelle placer l'unité, non {@code null}
	 * @param unit l'unité à enrôler, non {@code null}
	 * @throws NullPointerException     si {@code pos} ou {@code unit} est
	 *                                  {@code null}
	 * @throws IllegalArgumentException si {@code unit} est déjà présente dans cette
	 *                                  armée, si {@code pos} est déjà occupée, ou
	 *                                  si {@code unit} est un général alors que
	 ...
	 * @return le gabarit avec les marqueurs remplacés par les valeurs
	 *         correspondantes
	 */
	@Override
	public String resolve(String template) {
		return template.replace(":name:", this.belligerent.toString()).replace(":mvt:", "" + this.mvt.getActual());
	}

	/**
	 * Retourne {@code true} si cette armée est encore en état de combattre,
	 * c'est-à-dire si elle possède un général vivant et strictement plus d'une
	 * unité.
	 *
	...
	/**
	 * Retourne {@code true} si cette armée possède au moins une unité à une
	 * distance de 1 de {@code c}.
	 */
	 ...
	 * position associée.
	 *
	 * @param unitConsumer le traitement à appliquer à chaque couple (position,
	 *                     unité)
	 */
	 ...
	 * @param mvtConsumed le nombre de points de mouvement consommés par ce
	 *                    déplacement, doit être compris entre 1 et le mouvement
	 *                    disponible pour cette unité
	 * @throws IllegalArgumentException si aucune unité ne se trouve à
	 *                                  {@code fromPos}, ou si {@code mvtConsumed}
	 ...
	 * de sa force propre, du nombre d'unités alliées adjacentes (à une distance de
	 * 1) et d'un terme additionnel {@code term}.
	 *
	 * @param pos  la position de l'unité dont la puissance doit être calculée
	 * @param term un terme additionnel (par exemple lié au terrain ou au contexte
	 *             du combat)
	 * @throws IllegalArgumentException si {@code pos} ne correspond à aucune unité
	 *                                  de cette armée
	 */
